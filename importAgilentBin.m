function [timeVector, voltageVector] = importAgilentBin2(inputFilename)
...
for waveformIndex = 1:nWaveforms
    ...
    %for bufferIndex = 1:nWaveformBuffers
        ...
        % move the voltage data into voltageVector
        if ((bufferType == 1) | (bufferType == 2) | (bufferType == 3))
            % bufferType is PB_DATA_NORMAL, PB_DATA_MIN, or PB_DATA_MAX (float)
            voltageVector(:, waveformIndex) = fread(fileId, nPoints, 'float');
        elseif (bufferType == 4)
            % bufferType is PB_DATA_COUNTS (int32)
            voltageVector(:, waveformIndex) = fread(fileId, nPoints, '*int32');
        elseif (bufferType == 5)
            % bufferType is PB_DATA_LOGIC (int8)
            voltageVector(:, waveformIndex) = fread(fileId, nPoints, '*uint8');
        else
            % unrecognized bufferType read as unformated bytes
            voltageVector(:, waveformIndex) = fread(fileId, bufferSize, '*uint8');
        end
